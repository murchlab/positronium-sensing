...
- `02_ac_stark_shift_sweeps_fig2ef.ipynb`: fits a dispersive model to AC Stark shift sweeps and exports the Fig. 2e,f panels.
- `03_singlet_readout_corrected_fi_analysis.ipynb`: converts `t → α`, extracts `P(α)` and `FI(α)` (readout-corrected), and writes `data_analysis/*_original.csv`.
- `04_singlet_raw_fi_analysis.ipynb`: reconstructs raw probabilities using readout confusion matrices, extracts `FI(α)`, and writes `data_analysis/*_raw.csv`.
