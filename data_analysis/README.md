...
```mermaid
flowchart LR
  nb03["Notebook 03\n(readout-corrected FI)"] --> da["`data_analysis/`"]
  nb04["Notebook 04\n(raw reconstruction FI)"] --> da
  da --> nb05["Notebook 05\n(gate-corrected plots)"]
  da --> nb06["Notebook 06\n(combined SI + Fig. 3)"]
```
...
- Style tag at the end of the filename:
  - `*_original.csv`: readout-corrected probabilities (corrected for assignment errors).
  - `*_raw.csv`: reconstructed raw/uncorrected probabilities.
  - `*_corrected.csv`: gate-fidelity-corrected singlet curves used for figure assembly.
